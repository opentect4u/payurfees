...
foreach($_REQUEST['latefe2'] as $reg)
	{
		$reg1.=$reg."_";
	}	
...
?>
<form name="aa" action="pay_cash.php" method="post" enctype="multipart/form-data" class="form-horizontal row-fluid">
<input type="hidden" name="late_fee" value="<?php echo $reg2;?>"/>
<input type="hidden" name="order_id" value="<?php echo random_code(8);?>"/>
<input type="hidden" name="student_id" value="<?php echo $_REQUEST['student_id'];?>"/>
<?php $dt=explode("/",$_REQUEST['info_period']);?>
<input type="hidden" name="fee_code" value="<?php echo $dt['3'];?>"/>
<input type="hidden" name="fee_period" value="<?php echo $dt['0'];?>"/>
<input type="hidden" name="ins_id" value="<?php echo $student_data2['id'];?>"/>	
...
<input type="hidden" name="course" value="<?php echo $student_data22['course_name'];?>"/>	
	<input type="hidden" name="info_period" value="<?php echo $_REQUEST['info_period'];?>"/>										
 <input type="hidden" name="amt" value="<?php echo $_REQUEST['tot_amt'];?>"   />  
<input  class="btn btn-primary btn-large" type="submit" name="submit" value="Cash Payment" onClick="return test2();"/>
</form>

<p>&nbsp;</p>
<form name="aa" action="simplepay.php" method="post" enctype="multipart/form-data" class="form-horizontal row-fluid">
...
<?php $student_data22=mysql_fetch_array(mysql_query("select * from tbl_course where 1 and id='".$student_data['course_name']."'"));?>	
<input type="hidden" name="course" value="<?php echo $student_data22['course_name'];?>"/>
<input type="hidden" name="info_period" value="<?php echo $_REQUEST['info_period2'];?>"/>	
			<input type="hidden" name="ins_id" value="<?php echo $student_data2['id'];?>"/>									
 <input type="hidden" name="amt" value="<?php echo $_REQUEST['tot_amt'];?>"   />  
<input  class="btn btn-primary btn-large" type="submit" name="submit" value="Online Payment" onClick="return test1();"/>
