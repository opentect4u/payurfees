...
where pid='".addslashes($_SESSION['parent_id'])."' and id='".$_REQUEST['id']."'";
mysql_query($sql);
?>
...
return null;
 ...
 <?php $data=mysql_fetch_array(mysql_query("select * from tbl_student where  pid='".addslashes($_SESSION['parent_id'])."' and id='".$_REQUEST['id']."'"));?>      
        <div class="wrapper">
...
<div class="control-group">
											...
											<label class="control-label" for="basicinput"><strong>Name of the Institute</strong><font color=red>*</font> :</label>

											<div class="controls">
                                            
<select  name="ins_id" id="ins_id" class="span8"/>
<option value="">--select--</option>

<?php
$sp="select * from tbl_institute where 1 order by ins_name asc";
$rp=mysql_query($sp);
while($dp=mysql_fetch_array($rp))
{
?>

<option value="<?php echo $dp['id'];?>" <?php if($data['ins_id']==$dp['id']) { echo 'selected'; }?>><?php echo $dp['ins_name']?></option>
<?php
}
?>
</select>
...
<option value="">--select--</option>
<?php
$s3="select * from tbl_course where 1 and ins_id='".$data['ins_id']."' and pid='0'";
$r3=mysql_query($s3);
while($d3=mysql_fetch_array($r3))
{
?>
<option value="<?php echo $d3['id'];?>" <?php if($data['course_name']==$d3['id']) { echo 'selected'; }?>><?php echo $d3['course_name'];?></option>
